...
public class FileUpService {
	...
	private static final org.slf4j.Logger logger = LoggerFactory.getLogger(FileUpService.class);
	/*
	 * 업로드한 파일을 외부에서 보기 위해 (다운로드, 이미지 경로지정) 폴더를 개방해야 하는데 서버거 위치한 폴드의 resources 폴더
	 * 아래에 upload 폴더를 만들고 그 폴더만 외부에 공개를 할 예정이다.
	 * 
	 * 실제 서버가 가지고 있는 resources폴더의 위치를 알아야 할 필요가 있다 실제 서버의 폴도위치를 추출하기 위해서
	 * ServletContext를 이용할 예정
	 * 
	 * 
	 */
	@Autowired
	...
	public String fileup(MultipartFile upFile) {
		...
		serverPath = "C:/bizwork/upload/";
		logger.debug("SERVER PATH:" + serverPath);

		try {
			// 1. upFile에 내용이 없으면 더이상 진행금지
			if (upFile.isEmpty())
				return null;

			// 2. web에서 보낸 파일의 실제 이름
...
//			String fileDir = "C:/bizwork/upload";
			...
			// 5. 지금 내가 저장하기 위해서 사용할 폴더 (디렉토리)가 없냐??
			if (!dir.exists()) {
				dir.mkdirs();// 만들어라
			}
			 ...
			 * 악의적인 사용자가 기존 업로드된 파일 이름과 같은
			 * 이름의 다른 파일을 업로드하면
			 * 이미 업로드된 파일이 변경되어 버린다.
			 * 
			 * 서버에 저장할 파일 이름을 변환시켜야 한다.
			 */
			...
			// 6. 서버에 저장할 폴더와 파일 이름을 결합한 객체 생성
			...
			// java 1.8이상에서 사용되는 코드
			// 3,  7 ~ 10번 코드를 한줄로..
			upFile.transferTo(serverFile);
			
			// 11. original 파일이름을 controller로 return
